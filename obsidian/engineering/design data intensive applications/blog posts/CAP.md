...
# availability
- having multiple copies of data (master-slave, master-master, quorum, ...), if data is written to one of server, it also be written to another servers -> there is connection between 2 servers for replication
# network partition
![[Pasted image 20240517150725.png | 600]]
	- one of two
		- continue writing to database -> fully available in both servers (any change in one server does not appear in the other server) -> violate linearizability (consistency)
