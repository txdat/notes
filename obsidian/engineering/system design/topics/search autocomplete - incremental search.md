...
- steps:
	...
	- get all valid children (under found node), sort them and get top-k
- optimization:
...
# data gathering service
- when user types a query, updating data in real-time is not practical:
...
# query service
- api server gets trie data from cache and construct autocomplete suggestion. if data isn't in cache, replenish data (from trie db) back to the cache
- optimization:
