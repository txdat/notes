...
- redis' hyperloglog -> estimate/approximate the cardinality of set (count), and **doesn't support delete operation**
...
### nosql database
- apache cassandra supports partitioning using gossip protocol and consistent hashing -> highly available
...
- used as buffer the count update events from clients, database is asynchronously updated at periodic intervals -> use serverless function to query and update DB (**pull model**)
...
- separate count update events (source of truth) and aggregation count (increase storage usage)
...
- update operations must be idempotent for conflict-free synchronization, but redis' update operations are not idempotent, and non-trivial
...
- prevents data duplication and doesn't rely on the order of replications?
- relaxes consistency constraints, tolerates network partition (accepts writes to disconnected nodes, merges later) -> high performance/throughput, low latency for both reads and writes
- CRDT exchanges data and operations including ordering and causality
- 3 types
...
- each replica stores its own counter and update operations instead of global counter -> use vector clock/version to identify causality of operations among replicas
